...
static tgtadm_err backingstore_mgmt(int lld_no, struct mgmt_task *mtask)
{
	int err;
	switch (mtask->req.op) {
	case OP_SHOW:
		concat_buf_init(&mtask->rsp_concat);
		err = backingstore_show(&mtask->rsp_concat);
		concat_buf_finish(&mtask->rsp_concat);
		break;
	case OP_NEW:
		err = backingstore_new(mtask->req_buf, mtask->req_bsize);
		break;
#if 0
	case OP_DELETE:
		err = backingstore_delete(mtask->req_buf, mtask->req_bsize);
		break;
#endif
	default:
